...
public class HttpSecretUtils {
    ...
    public static byte[] getParamString(String[] args){
        ...
        LogCustom.i(Const.TAG.ZY_HTTP, "请求参数串：" + sb.toString());
        String temp = TEA.encryptByTea(sb.toString());
//        LogCustom.i(Const.TAG.ZY_HTTP, "请求参数是：" + temp);
        byte[] result = TEA.change(temp);
//        LogCustom.i(Const.TAG.ZY_HTTP, "请求参数是：" + Arrays.toString(result));
        return result;
    }

    public static byte[] getParamString(String[] args,boolean forsecret){
        LogCustom.i(Const.TAG.ZY_HTTP, "请求参数是：" + Arrays.toString(args) + " + " + Arrays.toString(ZYApplication.appendParams));
        StringBuffer sb = new StringBuffer();
        if(args==null||args.length%2!=0){
            return new byte[0];
        ...
        String temp = "";
        if (forsecret){
            temp = TEA.encryptByTea(sb.toString());
        }else {
            temp = sb.toString();
        }
        byte[] result;
        if (forsecret){
            result = TEA.change(temp);
        }else {
            result  = temp.getBytes();
        }

//        LogCustom.i("ZYS","传输参数："+Arrays.toString(result));
        return result;
    }
...
//            System.out.println("req:" + req);
    ...
    /*public static void main(String[] args){
        ...
        *//*String temp = TEA.encryptByTea("3543132351fgdfg");
        System.out.println("加密后："+temp);
        temp = TEA.decryptByTea(temp);
