...
public class EaseSidebar extends View {
    ...
    private void setHeaderTextAndscroll(MotionEvent event) {
        ...
        if (sectionForPoint(event.getY()) == 0) {
            //从#开始
            return;
        }
        ...
        String[] adapterSections = (String[]) sectionIndex.getSections();//适配器里记录的头字母的数组
        try {
            for (int i = adapterSections.length - 1; i > -1; i--) {
                if (adapterSections[i].equals(headerString)) {
                    if (mListView.getHeaderViewsCount() > 0) {
                        mListView.setSelection(sectionIndex.getPositionForSection(i) + 1);//已选择的圈友占了第一条的位置
                    } else {
