...
public class XRefreshView extends LinearLayout implements OnScrollListener{
    ...
    private float OFFSET_CUSTOM = 1.4F;
        ...
        if (attrs != null) {
            ...
            try {
                isHeightMatchParent = a.getBoolean(
                        R.styleable.XRefreshView_XRefreshView_isHeightMatchParent, true);
                isWidthMatchParent = a.getBoolean(
                        R.styleable.XRefreshView_XRefreshView_isWidthMatchParent, true);
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (callback!=null&&mContentView.isTop()&&deltaY>0&&callback.isChangeAble(false)){
                    return true;
                }else {
                    if (!isIntercepted) {
                    ...
                    if (!mPullLoading && !mReleaseToLoadMore && mContentView.isTop() && (deltaY > 0
                            || (deltaY < 0 && mHolder.hasHeaderPullDown()))) {
                        sendCancelEvent();
...
//            LogUtils.d("sendCancelEvent");
    ...
    public void setHeadMoveLargestDistence(int headMoveDistence) {
        if (headMoveDistence <= 0) {
            ...
            mHeadMoveDistence = ScreenHeight / 7;
        } else {
...
//                LogUtils.d("currentY=" + currentY + ";mHolder.mOffsetY=" + mHolder.mOffsetY);
    ...
    public void onScrollStateChanged(AbsListView view, int scrollState) {
        if (stateCallback!=null) stateCallback.XRVScrollStateChangedCallback(scrollState);
    }
    ...
    private XRefreshViewScrollStateChangedCallback stateCallback;

    public void setStateCallback(XRefreshViewScrollStateChangedCallback stateCallback) {
        this.stateCallback = stateCallback;
    }

    public interface XRefreshViewScrollStateChangedCallback{
        void XRVScrollStateChangedCallback(int scrollState);
    }
}